 ...
 *
 * Unified management of all lazy-loaded route components, implements code splitting, improves first-screen loading performance
 *
 * Note: Using React.lazy(() => import(...)) here is an allowed exception
 * - React.lazy is React's officially recommended code splitting method
 * - Used for route-level lazy loading to improve first-screen loading performance
 * - Type safety is guaranteed by React.lazy, will not cause type issues
 * - This is a framework-level feature, not dynamic import in business code
 *
 * ⚠️ Important: Must use string literals, cannot use variables
 * - webpack needs static analysis of import() paths, cannot handle variable expressions
 * - Using string literals can eliminate "Critical dependency" warnings
 * - Path configuration reference pages-path.config.ts, but use string literals directly in this file
 *
 * Usage guidelines:
 * - All route files must import lazy-loaded components from this file
 * - Page paths are uniformly configured in pages-path.config.ts (for documentation and type checking)
 * - Use string literals in this file, paths should match pages-path.config.ts
 * - Use path aliases (@/pages/*) instead of relative paths for easier maintenance
 * - When adding new pages, first add path configuration in pages-path.config.ts, then add lazy-loaded component in this file
 */
